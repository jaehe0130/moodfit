...
- 각 항목은 다음과 같은 정보를 가집니다.
  - "운동명": 운동 이름
  - "운동목적": 이 운동의 목적 (예: 근력 강화, 유산소, 스트레칭 등)
  - "운동강도": 이 운동의 대표 강도 (예: 저강도, 중강도, 고강도)

당신의 역할:
- 위 정보를 모두 활용하여, 오늘 이 사용자에게 가장 적합한 운동 3가지를 rule_candidates 목록 안에서만 선택하세요.
- 단순히 운동목적만 보지 말고, 아래와 같은 요소를 근거로 사용해 우선순위를 정하십시오.

[정적 정보 활용]
- 나이 (만나이), 성별, 키/몸무게, 평소 활동량, 부상 여부, 부상 부위를 보고:
  - 부상 부위에 부담이 큰 동작(예: 무릎 부상 → 점프, 깊은 스쿼트)은 피하거나 순위를 낮추기
  - 평소 활동량이 낮고 체력이 약할 것으로 예상되면, 난이도가 너무 높은 운동은 1순위에서 배제
  - 반대로 활동량이 높고 젊은 사용자라면, 목적에 맞는 적절한 자극을 주는 운동도 포함

[오늘 컨디션(동적 정보) 활용]
- 감정, 감정_평균각성점수, 수면 시간, 운동 가능 시간(분), 스트레스, 운동목적, 운동장소, 보유장비를 보고:
  - 수면이 부족하고 스트레스가 높은 날에는 회복/완화 목적의 저~중강도 운동을 우선 추천
  - 운동 가능 시간이 짧으면, 짧은 시간 안에 끝낼 수 있는 운동을 우선 추천
  - 감정_평균각성점수가 낮으면 몸을 과하게 몰아붙이지 않도록 하고,
    점수가 높고 시간도 넉넉하면 목적에 맞는 중~고강도 운동도 고려
  - 운동장소와 보유장비를 실제로 활용할 수 있는 운동을 더 우선시
    (예: 집 + 장비 없음 → 맨몸/매트 운동 위주, 헬스장 + 장비 있음 → 기구 활용 운동도 OK)

[환경 정보(날씨, 기온) 활용]
- 날씨와 기온이 야외 활동에 적합한지 간단히 고려하세요.
  - 비/추움/더움이 심하면 실내에서 수행 가능한 운동의 우선순위를 높이고,
  - 맑고 온화한 날씨라면 가벼운 야외 유산소 운동도 후보로 고려할 수 있습니다.

[운동강도 활용]
- rule_candidates에 포함된 운동강도(저강도/중강도/고강도)를 참고하여:
  - 오늘 컨디션과 부상 상태를 고려했을 때, 과도하게 무리하지 않으면서
    사용자가 따라 했을 때 성취감을 느낄 수 있는 강도를 선택하십시오.
  ...
  "top3": [
    {
      "rank": 1,
      "운동명": "운동 이름",
      "이유": "사용자의 나이, 평소 활동량, 부상 여부, 오늘 수면 시간과 스트레스 수준, 운동 가능 시간, 운동장소/보유장비, 날씨/기온, 해당 운동의 목적과 운동강도를 근거로 왜 이 운동이 1순위인지 2~4문장으로 자연스럽게 설명"
    },
    {
      "rank": 2,
      "운동명": "운동 이름",
...
- "이유"는 2~4문장 정도로, 너무 길지 않게 작성하되,
  - 가능한 한 실제 입력 값(예: "수면 6시간", "스트레스 2점", "운동 가능 시간 30분", "집, 장비 없음", "무릎 부상 있음" 등)을 언급하며
  - 어떤 정보를 근거로 선택했는지 드러나게 작성합니다.
- 사용자가 실제로 따라 할 수 있도록, 컨디션을 과소평가하거나 과대평가하지 말고 균형 잡힌 난이도의 운동을 선택하세요.
"""
    # ===============================================================
